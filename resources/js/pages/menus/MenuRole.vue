                        ...
                        <div class="card-title">
                            <select v-model="form.role_id" @change="loadMenuRole(form.role_id)"
                                ...
                                <option :value="role.id"
                                        v-for="role in form_props.roles">{{role.name}}</option>
                            </select>
                        </div>
                                ...
                                <div class="p-3" v-for="menu in form_props.menu_role">
                                    <div class="d-flex">
                                        <div class="form-check form-check-sm form-check-custom form-check-solid">
                                            <input class="form-check-input" type="checkbox" v-model="menu.selected" value="1">
                                        </div>
                                        <div class="ms-2" style="font-weight: bold">
                                            {{menu.title}} <span class="text-gray-600 font-italic" style="font-weight: normal">{{menu.url}}</span>
                                        </div>
                                    </div>
                                    <div v-if="menu.children" class="ms-10">
                                        <div v-for="cld in menu.children" class="d-flex pt-3">
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input" type="checkbox" v-model="cld.selected" value="1">
                                            </div>
                                            <div class="ms-2" style="font-weight: bold">{{cld.title}}
                                                <span class="text-gray-600 font-italic" style="font-weight: normal">{{cld.url}}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
export default {
    components: {CustomIcon, Breadcrumb},
    setup() {
